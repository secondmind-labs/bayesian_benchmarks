...
class RegressionModel(object):
    ...
    def _fit(self, X, Y, Lik, **kwargs):
        if X.shape[0] > self.ARGS.num_inducing:
            ...
            Z = np.concatenate(
                [X, np.random.randn(self.ARGS.num_inducing - X.shape[0], X.shape[1])], 0
            )
        ...
        if not self.model:
            ...
            for _ in range(2):
                kerns.append(
                    gpflow.kernels.RBF(
                        X.shape[1], lengthscales=float(X.shape[1]) ** 0.5
                    )
                )

            mb_size = (
                self.ARGS.minibatch_size
                if X.shape[0] > self.ARGS.minibatch_size
                else None
            )

            self.model = DGP(X, Y, Z, kerns, Lik(), minibatch_size=mb_size, **kwargs)

            self.model.layers[0].q_sqrt = (
                self.model.layers[0].q_sqrt.read_value() * 1e-5
            )

            if isinstance(
                self.model.likelihood.likelihood, gpflow.likelihoods.Gaussian
            ):
                ...
                self.ng = gpflow.train.NatGradOptimizer(
                    gamma=self.ARGS.gamma
                ).make_optimize_tensor(self.model, var_list=var_list)
            else:
                self.ng = None

            self.adam = gpflow.train.AdamOptimizer(
                self.ARGS.adam_lr
            ).make_optimize_tensor(self.model)
        ...
        self.model.layers[0].q_mu.assign(
            np.zeros((self.ARGS.num_inducing, X.shape[1])), session=self.sess
        )
        self.model.layers[0].q_sqrt.assign(
            1e-5 * np.tile(np.eye(self.ARGS.num_inducing)[None], [X.shape[1], 1, 1]),
            session=self.sess,
        )

        self.model.layers[1].feature.Z.assign(Z, session=self.sess)
        num_outputs = self.model.layers[1].q_sqrt.shape[0]
        self.model.layers[1].q_mu.assign(
            np.zeros((self.ARGS.num_inducing, num_outputs)), session=self.sess
        )
        self.model.layers[1].q_sqrt.assign(
            np.tile(np.eye(self.ARGS.num_inducing)[None], [num_outputs, 1, 1]),
            session=self.sess,
        )
        ...
        try:
            for _ in range(iters):
                if _ % 100 == 0:
                    print(
                        "{} {}".format(_, self.sess.run(self.model.likelihood_tensor))
                    )
                if self.ng:
...
class ClassificationModel(RegressionModel):
    ...
    def predict(self, Xs):
        m, v = self.model.predict_y(
            Xs, self.ARGS.num_posterior_samples
        )  # num_samples, N_test, K
