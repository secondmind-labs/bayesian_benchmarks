    ...
    def fit(self, X, Y):
        ...
        if not self.model:
            ...
            self.model = gpflow.models.SVGP(
                X, Y, kern, lik, feat=Z, minibatch_size=mb_size
            )
        ...
        self.model.q_mu.assign(
            np.zeros((self.ARGS.num_inducing, Y.shape[1])), session=self.sess
        )
        self.model.q_sqrt.assign(
            np.tile(np.eye(self.ARGS.num_inducing)[None], [Y.shape[1], 1, 1]),
            session=self.sess,
        )
...
class ClassificationModel(object):
    ...
    def fit(self, X, Y):
        Z = (
            kmeans2(X, self.ARGS.num_inducing, minit="points")[0]
            if X.shape[0] > self.ARGS.num_inducing
            else X.copy()
        )
